//コピペ:https://qiita.com/ushikunv/items/eebe435bef24c027ed16
//コード散在しているし解析したくない…。
//自作安定かなぁ
...
public class MeshCut : MonoBehaviour
{
    ...
    /// <summary>
    /// 切断したオブジェクトに追加するコンポーネント
    /// MEMO:下手に弄らない様に静的広域変数で定義
    /// </summary>
    public static Component[] addComponents;

    /// <summary>
    /// 切断してオブジェクトが誕生した時に実行するコールバック関数
    /// MEMO:設計がひどすぎる
    /// </summary>
    public static System.Action<GameObject> InstantiateFunc;
    ...
    public void Cut(Plane cutPlane)
    {
        ...
        if (addComponents != null)
        {
            ...
            foreach (var it in addComponents)
            {
                Debug.LogError("call2;"+it);
                obj.AddComponent(it.GetType());
            }
        }
        //生成後に実行する関数
        InstantiateFunc?.Invoke(obj);
        ...
        if (addComponents != null)
        {
            foreach (var it in addComponents)
            {
                var comp = obj2.AddComponent<Component>();
                comp = it;
            }
        }

        //生成後に実行する関数
        InstantiateFunc?.Invoke(obj2);
